...
export default function App() {
  ...
  const handlePdfUpload = async (file) => {
    ...
    setStampPos({
      x: lastCanvas.width / SCALE - 160,
      y: lastCanvas.height / SCALE - 110,
    });
  };
  ...
  const onMouseDown = useCallback(
    (e) => {
      if (e.target.dataset.resize) {
        setResizing(true);
        setResizeStart({ mouseX: e.clientX, mouseY: e.clientY, w: stampSize.w, h: stampSize.h });
        e.preventDefault();
        return;
      }
  ...
  const onMouseMove = useCallback(
    (e) => {
      ...
      if (resizing) {
        setStampSize({ w: Math.max(40, resizeStart.w + e.clientX - resizeStart.mouseX), h: Math.max(20, resizeStart.h + e.clientY - resizeStart.mouseY) });
        return;
      }
      const lastCanvas = pageCanvases[lastPageIndex];
      const canvasW = lastCanvas.width / SCALE;
  ...
  const handleExport = async () => {
    ...
    finalCanvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = "objednavka_razitko.png";
      a.click();
      ...
      {pdfLoaded && lastCanvas && (
        <div style={{ maxWidth: 780, margin: "0 auto" }}>
          ...
          <div style={{ position: "relative", display: "inline-block", border: "1px solid #2a2520", borderRadius: 6, overflow: "hidden", width: "100%", maxWidth: lastCanvas.width / SCALE, userSelect: "none" }}>
            ...
            {stampLoaded && (
              <div ref={overlayRef} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp} style={{ position: "absolute", inset: 0, cursor: dragging ? "grabbing" : "default" }}>
